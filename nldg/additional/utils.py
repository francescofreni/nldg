# code modified from https://github.com/zywang0701/DRoL/blob/main/methods/utils.py
import numpy as np
...
class OutcomeModel:
    ...
    def fit(self, X, Y, sample_weight=None, verbose=0, seed=None):
        ...
        if self.learner == "xgb":
            ...
            param_grid = {
                "learning_rate": [0.1],
                # [0.01, 0.05, 0.1],    # Step size shrinkage used in update to prevents overfitting
                "max_depth": [
                    3,
                    6,
                ],  # [3, 6, 9],          # Maximum depth of a tree
...
class DensityModel:
    ...
    def fit(self, X, X_target, verbose=0, seed=None):
        ...
        Y_concat = np.concatenate(
            (np.zeros(X.shape[0]), np.ones(X_target.shape[0]))
        )
        if self.learner == "logistic":
            self.model = LogisticRegression(solver="lbfgs").fit(
                X_concat, Y_concat
            )
            param_grid = None
        if self.learner == "xgb":
