...
class MaximinGPR(MultiOutputMixin, RegressorMixin, BaseEstimator):
    ...
    * exposes a method `log_marginal_likelihood(theta)`, which can be used
    ...
    def fit(self, X, E, y):
        ...
        E : array-like of shape (n_samples,) or list of object
            Environment labels.
        ...
        self.E_ = np.copy(E) if self.copy_X_train else E
    ...
    def log_marginal_likelihood(
        ...
        min_loglik = np.inf
        for env in np.unique(self.E_):
            X_train_e = self.X_train_[self.E_ == env]
            y_train_e = y_train[self.E_ == env]

            if eval_gradient:
                K, K_gradient = kernel(X_train_e, eval_gradient=True)
            else:
                K = kernel(X_train_e)
            ...
            min_log_likelihood = min(min_loglik, log_likelihood)
