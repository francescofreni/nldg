...
OUT_DIR = os.path.join(OUT_DIR, "comparison_gdro_magging_covariates")
os.makedirs(OUT_DIR, exist_ok=True)
...
def plot_maxrisk_vs_nenvs(
    ...
    for m in methods:
        ...
        for p in p_vals:
            vals = np.asarray(results[p][m], dtype=float)
            if vals.size == 0:
                continue
            n = vals.size
            mu = vals.mean()
    ...
    plt.savefig(
        os.path.join(
            OUT_DIR,
            f"{risk_label}_changeXdistr{str(CHANGE_X_DISTR)}_leaf{MIN_SAMPLES_LEAF}_reps{N_SIM}_numenv{NUM_ENV}.pdf",
        ),
        dpi=300,
        bbox_inches="tight",
    )
...
if __name__ == "__main__":
    ...
    for p in tqdm(Ps):
        ...
        results[p]["RF"] = max_risks[:, 0].tolist()
        results[p][f"MaxRM-RF({risk_label})"] = max_risks[:, 1].tolist()
        results[p][f"Group DRO({risk_label})"] = max_risks[:, 2].tolist()
        results[p][f"Magging({risk_label})"] = max_risks[:, 3].tolist()

    np.save(
        os.path.join(
            OUT_DIR,
            f"{risk_label}_changeXdistr{str(CHANGE_X_DISTR)}_leaf{MIN_SAMPLES_LEAF}_reps{N_SIM}_numenv{NUM_ENV}.npy",
        ),
        results,
    )
