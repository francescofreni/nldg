...
def run_experiment(
    ...
    if Xval is not None:
        results_val_df = (
            ...
            .pivot(index="EnvIndex", columns="Model", values="MSE")
            .reset_index()
...
def run_l5go_experiment(
    X, y, env, unique_envs, seed=42, folds=None, verbose=True, fold_size=5
):
    results_test = []
    preds_test = []
    ...
    for fold in range(len(folds)):
        ...
        if verbose:
            print(
                f"Running experiments with {test_counties} as test environments"
            )
        test_mask = np.isin(
            env, [unique_envs.get_loc(c) for c in test_counties]
        )
